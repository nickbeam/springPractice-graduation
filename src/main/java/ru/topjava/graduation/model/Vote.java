...
@NamedQueries({
        ...
        @NamedQuery(name = Vote.GET_BY_USER, query = "SELECT v FROM Vote v JOIN FETCH v.user JOIN FETCH v.restaurant WHERE v.user.id = :userId AND v.dateTime >= :startDateTime AND v.dateTime < :endDateTime ORDER BY v.dateTime"),
        @NamedQuery(name = Vote.GET_COUNT, query = "SELECT COUNT (v) FROM Vote v WHERE v.restaurant.id = :restaurantId AND v.dateTime >= :startDateTime AND v.dateTime < :endDateTime"),
})
@Entity
